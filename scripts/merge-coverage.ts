...
import { e2eCoverageOptions, entryFilter, sourceFilter, sourcePathResolver } from "../e2e/coverage-options";
...
async function main() {
  ...
  // Playwright's browser coverage captures), sourcePathResolver (stripping
  // Next's "_N_E/" webpack namespace prefix so sourceFilter's patterns can
  // match at all), and e2eAllOption (crediting files with zero coverage
  // instead of omitting them) are reused from e2eCoverageOptions so the
  // merged report's rules stay identical to the e2e-only report's.
  const merged = MCR({
